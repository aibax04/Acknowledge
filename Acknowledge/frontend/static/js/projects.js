...
// Handles all CRUD operations for projects and member management
...
async function fetchProjectDetails(projectId) {
    try {
        const response = await Api.get(`/ventures/${projectId}`);
        return response;
    } catch (error) {
...
async function deleteProject(projectId) {
    try {
        await Api.delete(`/ventures/${projectId}`);
        return true;
    } catch (error) {
...
async function fetchAvailableUsers(projectId) {
    try {
        const response = await Api.get(`/ventures/${projectId}/available-users`);
        return response || [];
    } catch (error) {
...
async function addMembersToProject(projectId, userIds) {
    try {
        const response = await Api.post(`/ventures/${projectId}/members`, { user_ids: userIds });
        return response;
    } catch (error) {
...
async function removeMemberFromProject(projectId, userId) {
    try {
        const response = await Api.delete(`/ventures/${projectId}/members/${userId}`);
        return response;
    } catch (error) {
...
function renderProjectsList(projects) {
    ...
    if (projects.length === 0) {
        container.innerHTML = `
            <div class="col-span-full text-center py-12 text-gray-500">
                ...
                <p class="text-sm">Create your first project to organize your team</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = projects.map(project => `
        <div class="bg-white border border-gray-100 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer project-card"
            ...
            <h3 class="font-semibold text-gray-900 mb-1">${escapeHtml(project.name)}</h3>
            <p class="text-sm text-gray-500 line-clamp-2">${escapeHtml(project.description || 'No description')}</p>
    ...
    container.querySelectorAll('.remove-member-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (confirm('Remove this member from the project?')) {
                ...
                openProjectModal(projectId); // Refresh
...
function openCreateProjectModal() {
    ...
    showModal(document.getElementById('create-project-modal'));
}

function closeCreateProjectModal() {
    hideModal(document.getElementById('create-project-modal'));
}
...
async function openAddMembersModal() {
    const projectId = document.getElementById('project-modal-id').value;
    const users = await fetchAvailableUsers(projectId);
...
function initProjectsModule() {
    ...
    // Confirm create
    const confirmCreateBtn = document.getElementById('confirm-create-project');
    if (confirmCreateBtn) {
    ...
    if (confirmAddMembersBtn) {
        confirmAddMembersBtn.addEventListener('click', async () => {
            ...
            const projectId = document.getElementById('project-modal-id').value;
            try {
                await addMembersToProject(projectId, selectedUsersToAdd);
    ...
    const notifyBtn = document.getElementById('btn-project-notify');
    if (notifyBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-tab="projects"]').forEach(link => {
