...
function renderAttendanceMonthly(data) {
    ...
    var h='<div class="grid grid-cols-5 gap-3 mb-6">';
...
async function loadPendingAttendanceRequests() {
    ...
    try{var reqs=await Api.get('/attendance/update-requests/pending');if(!reqs||reqs.length===0){c.innerHTML='<p class="text-sm text-gray-500 text-center py-4">No pending requests</p>';return;}
